...
<input type="range" id="hueSlider" min="0" max="360" value="0" />
 CORRUPTED ANIMATION, pretty cool 
<script>
  function changeHue(image, hueValue) {
    const canvas = document.createElement("canvas");
    const ctx = canvas.getContext("2d");
  ...
  function autoChangeHue() {
    let hueValue = 0;

    setInterval(() => {
      hueValue = (hueValue + 0.05) % 260; // Slower animation, adjust as needed
      changeHue(image, hueValue);
      hueSlider.value = hueValue;
    }, 150); // Change hue every 200 milliseconds (adjust as needed)
  }
